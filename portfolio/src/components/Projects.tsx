        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          {projects.map((project, index) => (
            <div
              ...
              onClick={() => {
                if (project.easterEgg === "flappy") setActiveEgg(index);
              }}
            >
              {/* hover shimmer */}
              ...
              {/* Card Content — slides down out of view when easter egg is active */}
              <div
                className="relative z-10 flex flex-col h-full transition-transform duration-500 ease-in-out"
                style={{ transform: activeEgg === index ? "translateY(150%)" : "translateY(0)" }}
              >
                {/* Top icons */}
              ...
              {/* Easter egg overlay — only renders when this card is active */}
              {activeEgg === index && project.easterEgg === "flappy" && (
                <FlappyBirdGame onClose={() => setActiveEgg(null)} />
              )}
            </div>
          ))}
        </div>
